...
#define REQUIRE(...) if (!(__VA_ARGS__)) { std::cout << __LINE__ << " error\n"; }
#define REQUIRE_FALSE(X) if((X)) { std::cout << __LINE__ << " error\n"; }
#define STR(x) STR2(x)
#define STR2(x) # x
#define REQUIRE_THROWS_AS(x, e) try { (x); throw std::runtime_error(STR(__LINE__) " error"); } catch (e&) {}
#define REQUIRE_NOTHROW(x) try { (x); } catch (...) {throw std::runtime_error(STR(__LINE__) " error");}
