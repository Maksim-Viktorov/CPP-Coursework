...
#define REQUIRE_THROWS_AS(x, e) try { if(x) {}; throw std::runtime_error("s"); } catch (e ex) {}

void CheckEqual(const String& actual, std::string_view expected) {
  std::cout << "CheckEqual(" << expected << ')' << '\n';
...
void CheckComparisonEqual(const String& lhs, const String& rhs) {
  std::cout << "CheckComparisonEqual(" << lhs << ", " << rhs << ')' << '\n';
  REQUIRE(lhs == rhs);
  REQUIRE(lhs <= rhs);
  REQUIRE(lhs >= rhs);
